...
import org.l2jasp.gameserver.data.CrownTable;
...
import org.l2jasp.gameserver.network.serverpackets.ShowCalculator;
...
public class UseItem implements ClientPacket
{
	...
	public void run(GameClient client)
	{
		...
		if (item.isEquipable())
		{
			...
			{
				player.sendPacket(new UserInfo(player)); // Asp: send UserInfo before ItemList.
				player.sendPacket(new ItemList(player, true)); // Asp: send ItemList after UserInfo.
			}
		}
		else
		{
